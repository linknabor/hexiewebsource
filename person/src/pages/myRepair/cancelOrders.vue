...
export default {
   ...
   methods: {
       ...
       storeMemo(e) {
        ...
        // 取消
        backPage() {
            vm.$router.push({path:'/ordersDetail',query:{orderId:vm.$route.query.orderId}})
        },
        //确定
        confirm() {
            var cancelReq = {
                orderId:vm.$route.query.orderId,
        	    cancelReasonType:vm.cancelReasonType,
        	    cancelReason: vm.cancelReason
            }
              vm.receiveData.postData(vm,'repair/cancel',cancelReq,'res',function(){
                if(vm.res.success) {  
                  alert("维修单已取消");
                  vm.$router.push({path:'/ordersDetail',query:{orderId:vm.$route.query.orderId}})
                }else {
