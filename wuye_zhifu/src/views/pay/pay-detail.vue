    ...
    <div class="pay-detail" v-if="selectUpton">
      ...
      <div class="user-info">
        ...
        <div class="addr ov">
          ...
          <p class="fr">
            {{area}} m
            <sup style="font-size:0.2rem">2</sup>
          </p>
        </div>
      </div>
      ...
      <dl v-for="item in feeList" class="fee-list" v-if="version=='01'">
        ...
        <dd class="ov">
          <p
            class="detail-date fl"
          >{{item.start_date | moment("YYYY年MM月DD日")}}-{{item.end_date | moment("YYYY年MM月DD日")}}</p>
      ...
      <ul class="discount">
        <li class="ov" @click="uptonList">
          <div class="fl">
            ...
            <span class="can-use">
              &nbsp;
              <strong>{{uptonData.length}}</strong>&nbsp;张可用&nbsp;
            </span>
          </div>
      ...
      <form class="invoice" v-show="show_invoice=='1'">
        ...
        <div class="form-row" v-show="invoice_title_type == '02'&&needInvoice=='1' ">
          ...
          <label class="t-label">
            发票抬头:
            <input type="text" placeholder="发票抬头" v-model="invoice_title" class="t-input" />
          </label>
        </div>
        <div class="form-row" v-show="invoice_title_type == '02'&&needInvoice=='1' ">
          ...
          <label class="t-label">
            公司税号:
            <input type="text" placeholder="公司税号" v-model="credit_code" class="t-input" />
          </label>
        </div>
...
export default {
  data() {
    return {
      ...
      feeList: "", //费用列表
      couponId: "", //优惠券id
      mianBill: "0", //停车优惠的账单id
      mianAmt: 0.0,
      fee_mianBill: "0", //物业优惠的账单id
      fee_mianAmt: 0.0,
      show_invoice_flag: "0",
      ...
      // before_condition: "", //判断停车是否减免
      // billfee_discount_rule: "", //物业费减免
      // billfee_discount_rule_conf: "", //物业费减免规则
      // park_discount_rule_conf: "", //停车减免
      // park_discount_rule: "",//停车减免规则
  ...
  created() {
    ...
    vm.version = this.$route.query.getversion; //根据版本号进行判断
  },
  ...
  methods: {
    ...
    getBillDetail() {
      if (vm.version == "01") {
        ...
        vm.receiveData.getData(vm, url, "data", function() {
          ...
          vm.show_invoice_flag =
            vm.data.result.other_bill_info[0].show_invoice_flag;
          ...
          vm.verNumber = useDate.ver_no; //地址
          vm.addr = useDate.cell_addr; //面积

          vm.area = useDate.cnst_area; //费用列表
    ...
    submit() {
      ...
      for (let i in vm.uptonData) {
        ...
        if (!flag) {
          vm.uptonAmount = "未使用";
          (vm.upronAmountNumber = 0), (vm.couponId = "");
          vm.calcReduceAmt();
        }
        vm.selectUpton = true;
      }
    ...
    wechatPay() {
      ...
      if (vm.version == "01") {
        url =
          ...
          this.end_date +
          "&couponUnit=" +
          vm.upronAmountNumber +
          "&couponNum=1&couponId=" +
          vm.couponId +
          "&mianBill=" +
          vm.mianBill +
          "&mianAmt=" +
        ...
        url =
          "getPrePayInfo?billId=" +
          vm.routeParams.billIds +
          "&stmtId=" +
          vm.routeParams.stmtId +
          "&regionname=" +
          this.regionname +
          ...
          wx.chooseWXPay({
            ...
            success: function(res) {
              ...
              vm.receiveData.postData(vm, reqUrl, {}, "reqUrlData", function() {
                ...
                //支付成功跳转详情
                var oriapp = vm.getUrlParam("oriApp")
                  ? "oriApp=" + vm.getUrlParam("oriApp")
                  : "";
