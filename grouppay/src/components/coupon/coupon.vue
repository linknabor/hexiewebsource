...
export default {
    ...
    methods: {
        ...
        determine() {
            if(vm.type == 12) {
                vm.$router.push({path:'/cardrollbuy',name:'cardrollbuy',query:{type:vm.type,ruleId:vm.ruleId,couponid:vm.couponid}});
            }else {
                vm.$router.push({path:'/confirmation',name:'confirmation',query:{type:vm.type,couponid:vm.couponid}});
            }
        }
    }
};
